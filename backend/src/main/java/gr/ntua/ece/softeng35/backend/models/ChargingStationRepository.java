...
public interface ChargingStationRepository extends JpaRepository<ChargingStation, Integer> {
   ...
   @Query(value= "SELECT u.id, u.quantity, u.quantityAvailable, u.quantityOperational, u.chargingSpot.id FROM ChargingStationSpots u WHERE u.chargingStation.id= ?1")
   List<List<Integer>> findStationSpotsFromStation(Integer stationId);

   @Query(value= "SELECT u.connectionType.category FROM ChargingSpot u WHERE u.id= ?1")
   String findConnCategory(Integer SpotId);
