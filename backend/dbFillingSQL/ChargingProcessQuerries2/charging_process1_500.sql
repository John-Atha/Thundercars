(SELECT `a`.`user`, `a`.`vehicle`, `b`.`station`, `b`.`spot`
      FROM
        ...
        WHERE `user`.`id` = 1
          ...
          AND( `dc_charger`.`id` = `vehicle`.`id` AND `dc_charger_port`.`dc_charger_id` = `dc_charger`.`id`)
          OR (`ac_charger`.`id` = `vehicle`.`id`
          AND `ac_charger_port`.`ac_charger_id` = `ac_charger`.`id`)ORDER BY rand()) AS `a`,
        ...
        WHERE `user`.`user_address_id` = `user_address`.`id`
          ...
          AND `connection_type`.`id` = `charging_spot`.`connection_type_id` ORDER BY rand() ) AS `b` 
